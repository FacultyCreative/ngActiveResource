...
angular
  ...
  .provider('ARGET', function() {
    this.$get = ['ARQuerystring', '$http', 'ARDeferred', 'ARAssociations', 'ARHelpers',
      function(querystring, $http, deferred, Associations, Helpers) {
      ...
      return function generateGET(instance, url, terms, options) {
          ...
          if (url.match(/\[\:\w+\]/)) url = url.replace(/\[\:\w+\]/, qs);
    ...
    this.$get = ['ARAPI', 'ARCollection', 'ARAssociation', 'ARAssociations',
      ...
      function Base() {
        ...
        _this.all = function() {
          ...
          return _this.where({}).then(function(results) {
            var deferred = $q.defer();
        ...
        function updateHasManyRelationship(collection, name) {
          var association = this[name.camelize()];
          emptyCollection(association);
        ...
        function addValidations(instance) { 
          ...
          instance.clearErrors = function(field) {
            instance.validates.clearErrors(field);
          }
        };
