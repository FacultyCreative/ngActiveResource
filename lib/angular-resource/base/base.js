...
angular
  ...
  .provider('ARSerializer', function() {
    this.$get = ['json', 'ARMixin', 'ARAssociations', 'ARHelpers', 'deferred',
...
angular
  ...
  .provider('ARMixin', function() {
    this.$get = function() {
      return function(receiver, giver, functions) {
        for (var i in giver) {
          if (functions) {
            mixinProp();
          } else {
...
angular
  ...
  .provider('ARBase', function() {
    this.$get = ['ARAPI', 'ARCollection', 'ARAssociation', 'ARAssociations',
      'ARCache', 'ARSerializer', 'AREventEmitter', 'ARValidations', '$http', '$q',
      '$injector', 'deferred', 
      function(API, Collection, Association, Associations, 
        Cache, Serializer, EventEmitter, Validations, $http, $q, 
        $injector, deferred) {
      ...
      function Base() {
        ...
        function resolveSingleGET(data, options) {
          ...
          return serializer.deserialize(data, instance, options);
        };
