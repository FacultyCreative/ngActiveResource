angular
  ...
  .provider('ARBase', function() {
    this.$get = ['ARAPI', 'ARCollection', 'ARAssociation', 'ARAssociations', '$http', '$q',
      'json', '$injector',
      function(API, Collection, Association, Associations, $http, $q, 
        json, $injector) {
      ...
      function Base() {
        ...
        function eagerLoad(instance) {
          var foreignKey    = instance.constructor.name.downcase() + '_id';
          var query         = {};
          query[foreignKey] = instance[primaryKey];
          return deferred(_.each(associations.hasMany, function(association) {
            association.klass.where(query).then(function(response) {
              _.each(response, function(associ) {
                instance[association.klass.name.pluralize().downcase()].push(associ);
              });
            });
          }));
        };
        ...
        base.$save = function(instance, url) {
          ...
          return $http.post(url, jsonified).then(function(response) {
            ...
            if (responseContainsForeignKeys(response)) return setAssociationsAndUpdate(instance, response, {lazy: true});
            return updateLocalInstance(instance, response, {lazy: true});
          });
        };
        ...
        function getBelongsToNames() {
          return _.map(associations.belongsTo, function(association) { return association.klass.name.downcase(); });
        };
        ...
        function getHasManyNames() {
          return _.map(associations.hasMany, function(association) { return association.klass.name.downcase().pluralize(); });
        };
        ...
        function getSettableProperties(model) { 
          ...
          var nonenumerables = ['establishBelongsTo', '$save', 'update', '$delete', 'associations', 'primaryKey'];
        ...
        _this.new = function(data) {
          ...
          _.each(associations.hasMany, function(collection) {
            instance[collection.klass.name.pluralize().downcase()][this.name.downcase()] = instance;
          }, this);
        ...
        function resolveMultiGET(data, options) {
          ...
          for (var i in data) {
            ...
            updateLocalInstance(instance, data[i], options);
          }
        ...
        function unlinkAssociations(instance) {
          _.each(associations.hasMany, function(hasManyInstance) {
            var associations = instance[hasManyInstance.klass.name.pluralize().downcase()];
        ...
        _this.find = function(terms, options) {
          ...
          else                      return generateGET(terms, options);
        };
