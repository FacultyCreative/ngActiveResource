...
simpleForm.directive('ngModel', ['$compile', function($compile) {
    ...
    compile: function() {
      return {
        post: function(scope, element, attrs, ctrls) {
          ...
          var $modelname       = attrs.ngModel.replace(/\.\w{0,}/g, '');
          ...
          scope.$watch($modelname, function(model) {
            ...
            $model.validate      = function(field) {
              originalValidate.call($model, field);
              setValid();
              setInvalid();
            };

            function setInvalid() {
              _.each($model.errors, function(error, field) {
              modelCtrl.$setValidity(field, false);
              });
            };

            function setValid() {
              _.each(fieldNames, function(field) {
                if (!$model.errors[field]) modelCtrl.$setValidity(field, true);
              });
            };
          });
