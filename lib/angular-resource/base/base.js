    ...
    this.$get = ['ARAPI', 'ARCollection', '$http', '$q', 'json', '$rootScope', '$injector',
      ...
      function Base() {
        ...
        _this.prototype.update = function(data) {
          ...
          var belongsToNames = _.map(belongsTo, function(klass) {
            return klass.name.downcase();
          });
          
          var settableProperties = getSettableProperties(_this);

          for (var attr in data) {
            if (_.include(hasManyNames, attr.downcase())) {
              if (data[attr].length > 0) this[attr] = _.map(data[attr], function(idata) { return new this.constructor[attr.classify(idata)]; }, this);
            } else if (_.include(belongsToNames, attr.downcase())) {
              if (data[attr].constructor.name == 'Number') delete data[attr];
              if (data[attr]) this[attr] = data[attr];
            } else {
              if (_.include(settableProperties, attr)) this[attr] = data[attr];
            }
          }
          return this;
        };
        ...
        _this.new = function(data) {
          if (typeof data == 'Number') data = {primaryKey: data};
