    ...
    this.$get = ['ARAPI', 'ARCollection', 'ARAssociation', 'ARAssociations', '$http', '$q',
      ...
      function Base() {
        ...
        function eagerLoad(instance) {
          var queries = [];
          _.each(associations.hasMany, function(association) {
            ...
            queries.push(function(callback) {
              association.klass.where(query, {lazy: true}).then(function(response) {
                _.each(response, function(associ) {
                  var name = association.klass.name.pluralize().downcase();
        ...
        base.$save = function(instance, url) {
          events.emit('$save:called', instance);
          ...
          return $http.post(url, jsonified).then(function(response) {
            ...
            function finish(instance) {
              ...
              events.emit('$save:complete', instance);
              return deferred(instance);
            }
          });
        };
        ...
        function setAssociationsAndUpdate(instance, response, options) {
          ...
          if (associations.belongsTo.length >= 1) {
            _.each(associations.belongsTo, function(foreignRel) {
              ...
              var queryOptions   = angular.copy(options);
        ...
        function aop(eventType, handler) {
        ...
        _this.before = function(eventType, handler) {
          return aop(eventType + ':called', handler);
        };

        _this.after = function(eventType, handler) {
          return aop(eventType + ':complete', handler);
        };
