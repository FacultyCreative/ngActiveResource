  ...
  .provider('ARBase', function() {
    this.$get = ['ARAPI', 'ARCollection', '$http', '$q', 'json', '$rootScope', '$injector',
      ...
      function Base() {
        ...
        // Similarly, belongsTo stores the belongsTo associations. 
        var belongsTo        = [];
        ...
        base.$save = function(instance, url) {
          ...
          return $http.post(url, jsonified).then(function(response) {
            ...
            if (responseContainsForeignKeys(response)) return setAssociationsAndUpdate(instance, response);
        ...
        function responseContainsForeignKeys(response) {
          var answer = false;
          _.each(belongsTo, function(foreignRel) {
            if (response[foreignRel.klass.name.downcase()]) answer = true;
          });
          return answer;
        };
        ...
        function setAssociationsAndUpdate(instance, response) {
          ...
          if (belongsTo.length >= 1) {
            _.each(belongsTo, function(foreignRel) {
              if (!response[foreignRel.klass.name.downcase()]) return;
              var association    = foreignRel.klass;
              var associatedName = association.name.downcase();
              var query          = response[associatedName];

              associations.push(function(callback) {
                foreignRel.klass.find(query).then(function(association) {
          ...
          async.series(associations, function(err, response) {
            response = _.first(response);
            updateLocalInstance(instance, response);
          });
          return deferred(instance);
        };
        ...
        function nameOfBelongsToModel(model) {
          ...
          if (!model.klass) return model.name.downcase();
          return model.klass.name.downcase();
        };
        ...
        _this.prototype.establishBelongsTo = function() {
          if (belongsTo.length) {
            for (var i in belongsTo) {
              var association     = belongsTo[i].klass;
              var associationName = nameOfBelongsToModel(association);
        ...
        function instanceBelongsTo(attr) {
          return _.include(getBelongsToNames(), attr.downcase());
        };
        ...
        function getBelongsToNames() {
          return _.map(belongsTo, function(association) { return association.klass.name.downcase(); });
        };
        ...
        function getHasManyNames() {
          return _.map(hasMany, function(klass) { return klass.name.downcase().pluralize(); });
        };
        ...
        function getSettableProperties(model) { 
          ...
          var properties = [];
          for (var prop in instance) { properties.push(prop); };
          var nonenumerables = ['establishBelongsTo', '$save', 'update', '$delete', 'hasMany', 'belongsTo', 'primaryKey'];
          _.remove(properties, function(prop) {
            return _.include(nonenumerables, prop);
          });
          if (!_.include(properties, primaryKey)) properties.push(primaryKey);
          return properties;
        };
        ...
        // function generateGET(terms, cached, multi)
        //
        // @param {terms} - JSON terms used to create a GET request. These will be parsed into a query
        //                  string.
        // @param {multi} - Boolean. If false, the GET request will return the first instance it finds
        //                  If true, it will return a collection of all instances that match the query.
        //
        ...
        // If `multi` is set to false, the first result found will be returned.
        function generateGET(terms, cached, multi) {
            var querystring = '?';
        ...
        function resolveSingleGET(data) {
          ...
          var instance = _this.new(data);
          if (responseContainsForeignKeys(data)) return setAssociationsAndUpdate(instance, data);
        ...
        function unlinkAssociations(instance) {
          _.each(hasMany, function(hasManyInstance) {
            var associations = instance[hasManyInstance.name.pluralize().downcase()];
            var name         = instance.constructor.name.downcase();
            _.each(associations, function(association) { association[name] = undefined; });
          });
          _.each(belongsTo, function(belongsInstance) {
            var association = instance[belongsInstance.klass.name.downcase()];
        ...
        _this.where = function(terms) {
          ...
          return generateGET(terms, cached, true);
        };
        ...
        _this.prototype.hasMany   = function(table, options) {
          ...
          if (!_.include(hasMany, klass)) hasMany.push(klass);
        ...
        _this.prototype.belongsTo = function(table, options) {
          ...
          if (options.foreignKey) var foreignKey = options.foreignKey;
          else                    var foreignKey = table.singularize() + "_id";

          var provider = $injector.get(providerName);
          if (!_.include(belongsTo, {klass: provider, foreignKey: foreignKey})) {
            belongsTo.push({klass: provider, foreignKey: foreignKey});
          }
          ...
          Object.defineProperty(this, table, {
            ...
            set: function(val) {
             ...
             if (val.constructor.name == 'String' || val.constructor.name == 'Number') {
               var instance = this;
               provider.find(val).then(function(response) { 
                 localTable = response; 
             ...
             } else if (provider.name == val.constructor.name) {
               localTable = val;
        ...
        _this.prototype.primaryKey = function(key) {
          primaryKey = key;
        };
